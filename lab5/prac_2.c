 ...
 * 
 * < Saagar Takhi - 201101203@daiict.ac.in >
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
  ...
  void eval(char *cmdline)
{
	...
	/*	if(sigemptyset(&mask) != 0)
		...
		//child, unblock mask, new group process, run command
		if(!pid)
		{
...
void do_bgfg(char **argv) 
{
	...
	if(pjid[0]=='%')
	{
		...
		{
			printf("%s No such job", pjid);
			return;
		}
	}
	//pid
	else if(isdigit(pjid[0]))
	{
...
void waitfg(pid_t pid)
{
	...
	{sleep(100);//for tsh to run in S+ state rather than R+, the parameter to sleep should be positive, eg. 1.
...
void sigtstp_handler(int sig) 
{
	...
	//send fg job process group sigstp
	if(pid != 0)
	{
